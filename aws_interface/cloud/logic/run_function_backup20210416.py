...
def run(server_class=HTTPServer, handler_class=S, port=20131):
    global can_run_subprocess
    server_address = ('', port)
...
def do(data, resource):
    ...
    if len(items) == 0:
        ...
        try:
            ...
            #     exe.submit(run)
            #     while not can_run_subprocess:
            #         pass
            #     future = exe.submit(run_subprocess, virtual_handler_path)
