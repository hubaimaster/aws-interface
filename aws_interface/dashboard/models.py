...
class User(models.Model):
    ...
    c_aws_access_key = models.CharField(max_length=512, blank=False)
    c_aws_secret_key = models.CharField(max_length=512, blank=False)

    @classmethod
    def create(cls, email, password, aws_access_key, aws_secret_key):
        from dashboard.security.crypto import AESCipher, Hash
        aes = AESCipher(password)
        password_hash = Hash.sha3_512(password)
